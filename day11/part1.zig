...
fn bfs_1_2(
    ...
    var queues = [3]Queue{ Queue{}, Queue{}, Queue{} };

    var node = Queue.Node{ .data = source };
    queues[0].append(&node);

    var qi: u4 = 0;
    while (queues[0].len + queues[1].len + queues[2].len > 0) : (qi = (qi + 1) % 3) {
        while (queues[qi].len != 0) {
            ...
            for (0..4) |dir| {
                ...
                if (distance[ni][nj] > distance[i][j] + weights[dir]) {
                    ...
                    queues[distance[ni][nj] % 3].append(next);
                }
            }
        }
    }
...
pub fn main() !void {
    ...
    for (0..positions.len) |i| {
        const dist = try bfs_1_2(positions[i], grid, big_rows, big_cols, &allocator);
